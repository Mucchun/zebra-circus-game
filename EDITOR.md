...
Open [zebra-scene-editor.timofeymarkin98.workers.dev](https://zebra-scene-editor.timofeymarkin98.workers.dev). Enter your name and the access code Tim shares privately, then wait for **Live**. No engine checkout or local setup is required. The v0.19.3 asset-enabled release uses a dedicated private Standard R2 bucket with executable free-tier ceilings and a fail-closed deployment check.
